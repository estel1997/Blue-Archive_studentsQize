...
このリポジトリは、就職活動で見せる iOS ポートフォリオとして、画面構成、状態管理、API 通信、ランキング保存、エラー処理を説明できるように整理しています。
...
- SwiftUI / API 通信の学習成果を確認したい採用担当者・エンジニア
...
API キーや secret の実値は README / docs には記載しません。公開前提の anon key を使う場合でも、RLS と公開範囲を確認して運用します。

詳しくは [docs/api-design.md](docs/api-design.md) を参照してください。
...
- API レスポンスを Swift のモデルに合わせて decode すること
- クイズ進行中の状態を破綻させずに画面へ反映すること
- ヒント開放、スコア、正誤判定の責務を分けること
- 通信失敗時のユーザー向けエラー表示を考えること
...
このリポジトリは学習・ポートフォリオ目的の非公式ファン制作物です。画像・音声・ゲーム素材など、権利物の追加は行いません。API キー、service role key、個人情報の実値を README / docs に記載しない方針です。
